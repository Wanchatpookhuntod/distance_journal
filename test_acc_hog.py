...
if __name__ == '__main__':
    ...
    while True:
        ...
        if face:
            ...
            cv2.putText(frame, f"Distance {number_dis}: {dis:.2f} cm. ", (20, 20), font, 0.5, color_value["white"], 1)

            list_dis.append(dis)
            
        time = time_processing(start)
        time_show = f'{text_method} >> {number_dis} | elapsed time: {time:.2f} ms/f'

        cv2.putText(frame, time_show, (20, 40), font, 0.5, color_value["white"], 1)
        cv2.imshow("output", frame)
        if cv2.waitKey(1) == 27 or number_dis == 500:
            break
        

    np.savetxt(f"csv/{text_method}_60New_{'accuracy.csv'}", list_dis)
