...
Builder.load_file("helper/roi.kv")
Window.size = (480, 800)
...
class Show_text(Widget):
    x_face= NumericProperty(0)
    y_face= NumericProperty(0)
...
class Main(Widget):
    ...
    def risk_show(self, dis):
        if self.distance:
            self.risk_status_text = "Good! View" if dis == 0 else "You have distance Risk !"
        self.show_text.risk_text = self.risk_status_text

    def noti(self, risk_status, dis):
        if self.frame_num % 60 == 0:
            if risk_status == 1:
                notification.notify(
                    title= "Risk !",
                    message=f"You have distance {round(dis)} cm",
    ...
    def working(self, start, choose, frame):
        ...
        if start == 1:
            ...
            if eyes:
                ...
                # Draw in kv
                self.show_text.x_face = int(self.face_detect.x*(3/4))
                self.show_text.y_face = int(self.face_detect.y *(3/4))
                self.kv_oncv.text = f"# {self.choose}\n {round(self.distance['distance'])} cm"
            else:
                self.reset_value()
